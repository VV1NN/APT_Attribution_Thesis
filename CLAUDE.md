...
### R-GCN Link Prediction (2026-04-06, 實作完成待訓練)

> 腳本：`scripts/rgcn_link_prediction.py`
> 結果：`scripts/results/lp_rgcn_distmult_protocol_A_random.json`（待產出）
...
**vs ComplEx baseline 的差異：**

| | ComplEx (已有) | R-GCN + DistMult |
|---|---|---|
| Node embedding | 獨立 lookup table | 由鄰居 message passing 聚合 |
| Node features | 不使用 | VT metadata (detection, type_tag, registrar, ASN...) |
| Multi-hop pattern | 無 | 2 層 = 2-hop |
| Inductive (新 node) | 無法處理 | 有 features 就能算 |
| 參數量 | ~17M (66K×128×2) | ~4.3M |

**效能預估：**
- MPS (macOS Apple Silicon)：~20 秒/epoch → 100 epochs ≈ 33 分鐘
- CUDA (RTX 4070)：~2-5 秒/epoch → 100 epochs ≈ 4-8 分鐘
...
# R-GCN + ComplEx decoder
uv run python scripts/rgcn_link_prediction.py --decoder complex --epochs 100 --dim 128

# R-GCN hyperparameter tuning
uv run python scripts/rgcn_link_prediction.py --dim 256 --n-bases 8 --lr 0.0005 --epochs 150
```
...
- [x] ~~Non-Neural Baselines~~ → Signal confirmed (ComplEx MRR=0.335)
- [x] ~~R-GCN 實作~~ → `scripts/rgcn_link_prediction.py`（待訓練比較）
- [ ] **R-GCN 訓練 + 結果比較** → Protocol A, 目標超越 ComplEx MRR=0.335
- [ ] **Temporal experiments** → Protocol B 三變體（R-GCN inductive 能力是關鍵優勢）
- [ ] **Prediction-Driven Attribution** → 預測鄰域 → overlap → AP@Coverage
- [ ] **Ablation** → temporal encoding / 因果邊 vs 全部 / TTP 語義節點 / node features 消融
