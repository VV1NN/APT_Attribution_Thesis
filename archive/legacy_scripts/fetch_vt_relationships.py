...
Rate limit: 4 req/min, 5800 lookups/day (academic limited plan).
"""
...
class RateLimiter:
    def __init__(self, requests_per_min: int = 4, daily_limit: int = 5700):
        self.min_interval = 60.0 / requests_per_min   # 16 s at default 3.75 req/min
...
def run(
    ...
    rate_limiter = RateLimiter(requests_per_min=2.0, daily_limit=daily_limit)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5700,
        help="Daily API call budget (default: 5700)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Search dirs for .env: virustotal_apiTest, project root
    script_dir = Path(__file__).parent
    env_search = [script_dir / "virustotal_apiTest", script_dir]
