...
| `contacted_url` | file → domain | 檔案執行時連線的 URL（提取 domain） | VT sandbox 行為分析 |
| **File → File** | | | |
...
| **反向關聯（Domain/IP → File）** | | | |
| `communicating_file` | domain/ip → file | 與該 domain/IP 通訊的檔案 | VT 通訊記錄 |
| `referrer_file` | domain/ip → file | 參照（引用）該 domain/IP 的檔案 | VT 參照記錄 |

> **程式碼中已定義但目前不會產生資料的邊類型**（VT academic plan 不支援或資料集未觸發）：
> `embedded_domain`, `embedded_ip`, `embedded_url`（靜態嵌入字串）、
> `itw_domain`, `itw_ip`, `itw_url`（In-the-Wild 分佈點）、
> `downloaded_file`（Enterprise-only）、`compressed_parent`（壓縮檔父檔案）。
> 這些邊類型在 `fetch_vt_relationships.py` 中已被註解（403 on academic plan），
> 但 `build_knowledge_graph.py` 保留了處理邏輯，未來升級 API 方案後可自動啟用。

#### 各邊類型在已建圖譜的實際分佈

| 邊類型 | 數量 | 佔比 |
|--------|------|------|
| `resolves_to` | 4,905 | 22% |
| `contacted_ip` | 2,947 | 13% |
| `referrer_file` | 2,941 | 13% |
| `dropped_file` | 2,698 | 12% |
| `bundled_file` | 2,097 | 10% |
| `has_ioc` | 1,719 | 8% |
| `communicating_file` | 1,446 | 7% |
| `contacted_domain` | 1,389 | 6% |
| `has_subdomain` | 1,041 | 5% |
| `contacted_url` | 608 | 3% |
| `execution_parent` | 188 | 1% |
| **合計** | **21,979** | **100%** |

> 統計基於 11 個已建 KG（APT1, APT-C-23, APT-C-36, APT12, APT16, APT17, APT18, APT19, APT28, APT29, Kimsuky）。
...
#### 邊屬性（Edge Attributes）說明

Layer 2 邊的 `attributes` 由 `_extract_edge_attrs()` 從 VT relationship response 中提取，非空：

| 屬性欄位 | 型別 | 適用邊類型 | 說明 |
|----------|------|-----------|------|
| `resolution_date` | `string` (ISO 8601) | `resolves_to` | DNS 解析日期 |
| `malicious` | `int` | 所有有 `last_analysis_stats` 的邊 | 目標節點的惡意偵測引擎數 |
...
#### contacted_ip / contacted_domain / contacted_url（Layer 2）

File 執行時的網路行為（VT sandbox 動態分析）。`contacted_url` 會從 URL 提取 domain 建立 domain 節點。
...
```json
{
  ...
  "attributes": {
    "malicious": 3,
    "undetected": 28,
    "last_analysis_date": "2026-03-01T10:00:00Z"
  }
}
```
...
{
  "source": "file_9200f80c...",
  "target": "domain_evil.example.com",
  "relationship": "contacted_url",
  "attributes": {
    "malicious": 10,
    "undetected": 15,
    "last_analysis_date": "2026-02-20T08:30:00Z"
  }
}
```

#### dropped_file / execution_parent / bundled_file（Layer 2）

File 間的衍生關係。

- `dropped_file`：執行後釋放的子檔案（VT sandbox）
- `execution_parent`：誰執行了這個檔案（VT 行為分析，反向關係）
- `bundled_file`：封裝/壓縮檔內包含的子檔案（VT 靜態分析）
...
```json
{
  ...
  "attributes": {
    "malicious": 45,
    "undetected": 8,
    "last_analysis_date": "2026-01-15T12:00:00Z",
    "type_tag": "peexe",
...
```json
{
  "source": "file_abc123...",
  "target": "file_def456...",
  "relationship": "execution_parent",
  "attributes": {
    "malicious": 30,
    "undetected": 12,
    "last_analysis_date": "2026-02-10T09:00:00Z"
  }
}
```

#### resolves_to / has_subdomain（Layer 2）

DNS 相關關係。

- `resolves_to`：DNS A record 解析歷史（Domain ↔ IP 雙向）
- `has_subdomain`：子域名關係（Domain → Domain）
...
```json
{
  ...
  "attributes": {
    "resolution_date": "2025-06-15T00:00:00Z",
    "malicious": 3,
    "undetected": 28,
    "last_analysis_date": "2026-03-01T10:00:00Z"
  }
...
```json
{
  "source": "domain_example.com",
  "target": "domain_mail.example.com",
  "relationship": "has_subdomain",
  "attributes": {}
}
```

#### communicating_file / referrer_file（Layer 2）

Domain/IP 的反向檔案關聯。

- `communicating_file`：與該 domain/IP 通訊的惡意檔案
- `referrer_file`：在內容中引用（reference）該 domain/IP 的檔案

```json
{
  "source": "domain_evil.example.com",
  "target": "file_9200f80c...",
  "relationship": "communicating_file",
  "attributes": {
    "malicious": 59,
    "undetected": 12,
    "last_analysis_date": "2026-03-13T06:25:18Z"
  }
}
```
...
```json
{
  "source": "ip_203.0.113.50",
  "target": "file_abc123...",
  "relationship": "referrer_file",
  "attributes": {
    "malicious": 42,
    "undetected": 5,
    "last_analysis_date": "2026-02-28T14:00:00Z"
  }
}
```
...
| 項目 | 值 |
|------|------|
| 每分鐘請求數 | 20,000 req/min |
| 每日額度 | 20,000 lookups/day |
| 每月額度 | 620,000 lookups/month |
| 存取層級 | Limited（standard free public API） |
| 使用限制 | 不得用於商業用途 |

> 群組配額由同群組所有使用者共享，任一使用者或總和達上限即暫停至下一時間窗口。

### 各腳本的 Rate Limiting 策略

#### `build_knowledge_graph.py`

| 項目 | 值 |
|------|------|
| 請求間隔 | 0.1 秒（`RATE_LIMIT_SEC = 0.1`，即 ~600 req/min，保守使用） |
| 429 重試 | 最多 3 次（`MAX_RETRIES = 3`），間隔遞增（0.1s × attempt） |
| 連線方式 | `requests.Session()`（Connection Pooling） |

#### `fetch_vt_relationships.py`

| 項目 | 值 |
|------|------|
| 請求間隔 | 0.1 秒（`requests_per_min=600`，即 `min_interval = 0.1s`） |
| 每日額度 | 18,000 lookups/day（程式內預設，預留 buffer） |
| 429 重試 | **無限次重試**，exponential backoff（60s → 120s → 300s → 600s） |
| 網路/5xx 重試 | 最多 3 次，每次等待 30s |
| 連線方式 | `requests.Session()`（Connection Pooling） |
